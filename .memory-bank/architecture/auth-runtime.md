...
|---|---|---|
| Next.js auth UI/state | Login choices, safe return path, customer/session status, completion UI, logout, and checkout gate. | No provider secrets or browser-persisted tokens. |
...
9. Storefront renders authenticated session state from the current-customer
   response. Guests and expired sessions retain no prior customer state.
...
- Stored user metadata: normalized email and optional first/last name from the
...
- The authenticated current-customer projection must not expose provider tokens,
